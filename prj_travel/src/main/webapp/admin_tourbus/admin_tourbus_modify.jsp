...
<%
	...
	try{
		if( tourId != null ){
			TourBusVO tbVO = tbmDAO.selectTourBus(tourId);
			
			String[]  routes = tbVO.getRoutes().split(",");
			String[] dispatchTime = tbVO.getDispatchTimes().split(" ");
			
			pageContext.setAttribute("tbVO", tbVO);
			pageContext.setAttribute("dispatchTime", dispatchTime);
			pageContext.setAttribute("routes", routes);
			 ...
			 $("#complete").click(function() {
				// File이 선택되지 않았을 경우
				 if ($("#chooseImg").get(0).files.length === 0) {
					 ...
					 endTime = $("#endDate").val();
					 busTourTime = $("#busTourTime").val();
					 tel = $("#tel").val();
					 adultFare = $("#adultFare").val();
					 childFare = $("#childFare").val();
					 slope = $("#slope").val();
					 
					 var dispatchTime = "";
					 $(".dispatchTime").each(function(){
						 dispatchTime += $(this).val() + " "
					 })
					 ...
					 $("#sub_name").val(name);
					 $("#sub_startTime").val(startTime);
					 $("#sub_endTime").val(endTime);
					 $("#sub_busTourTime").val(busTourTime);
					 $("#sub_tel").val(tel);
					 $("#sub_adultFare").val(adultFare);
					 $("#sub_childFare").val(childFare);
					 $("#sub_img").val(img);
					 $("#sub_dispatchTime").val(dispatchTime);
					 $("#sub_route").val(route);
  ...
  <!-- /.navbar -->
		...
		<div class="content-wrapper" >
            ...
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">투어버스 추가</li>
            </ol>
          </div>
						<!-- /.col -->
					</div>
    ...
    <div class="upload-box col-md-5">
      <div class="drag-file" id="drop-file">
        <img src="${ tbVO.image }" id="preview" style="display : inline">
      </div>
  	...
  	<input type="text" class="inputBox" id="name" name="name" style="width: 60%;display: inline-block; margin-left: 70px" placeholder="투어이름을 입력해주세요." value="${ tbVO.name }"><br/>
  	<label style="margin-left: 30px; margin-top: 30px; margin-right: 70px">운행시간</label>
  	 <input type="text" id="startDate" name="startTime" placeholder="운행시작 날짜 입력" value="${ tbVO.startTime }">
    <span style="margin-left: 30px">~</span>
    <input type="text" id="endDate" name="endTime" style="margin-left: 45px " placeholder="운행종료 날짜 입력" value="${ tbVO.endTime }">
    <br/>
  	<label style="margin-left: 30px; margin-top: 30px">소요시간</label>
  	<input type="text" class="inputBox" id="busTourTime" name="busTourTime" style="width: 60%;display: inline-block; margin-left: 70px; margin-top: 30px" placeholder="소요시간을 입력해주세요." value="${ tbVO.busTourTime }"><br/>
  	<label style="margin-left: 30px; margin-top: 30px">연락처</label>
  	<input type="text" class="inputBox" id="tel" name="tel" style="width: 60%;display: inline-block; margin-left: 85px; margin-top: 30px" placeholder="연락처를 입력해주세요." value="${ tbVO.tel }"><br/>
  	</div>
  </div>
			</td>
            </tr>
  	...
  	<input type="text" class="inputBox" id="adultFare" name="adultFare" style="width: 60%;display: inline-block; margin-left: 140px" placeholder="내용을 입력해주세요." value="${ tbVO.adultFare }"><br/>
  	<label style="margin-left: 30px; margin-top: 30px">소인 및 청소년</label>
  	<input type="text" class="inputBox" id="childFare" name="childFare" style="width: 60%;display: inline-block; margin-left: 70px; margin-top: 30px" placeholder="내용을 입력해주세요." value="${ tbVO.childFare }"><br/>
  		</div>
            </td>
            </tr>
            <tr>
            ...
            <label style="margin-left: 30px; float: left">배차시간</label>
  			...
  			<c:forEach var="time" items="${ dispatchTime }" >
  				<div class="input-wrapper" style="margin-left:180px; margin-bottom:20px;"></div>
    	       <input type="text" style="margin-right:60px; width: 50%" placeholder="시간을 입력해주세요." class="dispatchTime" name="dispatchTime" value="${ time }">
    	      <button class="remove-btn" onclick="dispatchRemoveField(this.parentNode)">&times;</button>
    	      </c:forEach>
            </div>
            <div class="contents" style="margin-top: 10px; display: inline-block; width: 60%;" id="routeInput">
  			...
  			<c:forEach var="route" items="${ routes }" >
  				<div class="input-wrapper" style="margin-left:180px;"></div>
             	<input type="text" style = "margin-right:60px; width: 60%; margin-bottom: 20px" placeholder="운행노선을 입력해주세요." class="route" name="route" value="${ route }">
             	<button class="remove-btn" onclick="routeRemoveField(this.parentNode)">&times</button>
  			</c:forEach>
            </div>
            </div>
            </td>
            ...
            <form action="http://192.168.10.133/prj_travel/admin_tourbus/admin_tourbus_update_proccess.jsp" id="sub_frm" method="post">
              	<input type="hidden"  id="sub_name" name="sub_name" />
              	<input type="hidden"  id="sub_startTime" name="sub_startTime" />
              	<input type="hidden"  id="sub_endTime" name="sub_endTime" />
              	<input type="hidden"  id="sub_busTourTime" name="sub_busTourTime" />
              	<input type="hidden"  id="tel" name="tel" />
              	<input type="hidden"  id="sub_adultFare" name="sub_adultFare" />
              	<input type="hidden"  id="sub_childFare" name="sub_childFare" />
              	<input type="hidden"  id="sub_dispatchTime" name="sub_dispatchTime" />
              	<input type="hidden"  id="sub_route" name="sub_route" />
              	<input type="hidden"  id="sub_img" name="sub_img" />
              	<input type="hidden"  id="sub_tourId" name="sub_tourId" value="${ tourId }" />
              	</form>
            </div>
            </div>
            </div>
